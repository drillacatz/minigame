(function (DD) {
  ...
  class DirectionDashGame {
    ...
    _loadRanking() {
      const order = DD.CONFIG.DIFF_ORDER;
      let store = {};
      try {
      ...
      return order
        .map((key) => {
          ...
          return { key, label: DD.CONFIG.DIFFICULTIES[key].label, score };
        })
    ...
    _setupGame() {
      ...
      this._input = new DD.InputManager(
        ...
        (dir, mode) => DD.Renderer.flashInputFeedback(dir, mode)
      );
    ...
    _runCountdown(stepIndex) {
      ...
      if (stepIndex < seq.length) {
        const isFinalStep = stepIndex === seq.length - 1;
    ...
    _handleWin() {
      ...
      const isHighScore = this._saveHighScore(score);
    ...
    _bindPermanentUI() {
      ...
      document.addEventListener('keydown', (e) => {
        ...
        if (this._state && (this._state.phase === 'win' || this._state.phase === 'lose')) {
          document.getElementById('js-btn-restart')?.click();
        } else if (DD.Renderer.isMenuOpen()) {
          document.getElementById('js-btn-start')?.click();
        }
      });
    }
  }

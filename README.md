...
│   ├── audio.js                    # ← Shared sound engine (SFX + BGM), used site-wide
...
└── assets/
    ...
    └── audio/
        ├── sfx/                    # correct.wav, wrong.wav, click.wav
        └── bgm/                    # lobby.wav (loops on the homepage)
```
...
One shared module — `js/audio.js` — powers all sound across the whole site:
correct/wrong gameplay feedback, the lobby background music, and click SFX
on buttons/links. It's included on every page (homepage + every game).

```
assets/audio/
├── sfx/
│   ├── correct.wav     # played on every correct input
│   ├── wrong.wav       # played on every wrong input
│   └── click.wav       # played on any element with data-sfx="click"
└── bgm/
    └── lobby.wav        # looping background music on the homepage
```

The files shipped above are synthesized placeholders so everything works
immediately — **swap them for your own .mp3 or .wav** any time:

1. Drop your file into `assets/audio/sfx/` or `assets/audio/bgm/`, using the
   **same filename** (e.g. replace `correct.wav` with your own `correct.wav`
   or `correct.mp3` — just update the path in the `MANIFEST` inside
   `js/audio.js` if you change the extension).
2. That's it — no other code changes needed.

**Adding click SFX to a new button:** add `data-sfx="click"` to any
`<button>` or `<a>` and `js/audio.js` plays the sound automatically:
```html
<button data-sfx="click">Some Button</button>
```

**Playing a sound from JavaScript** (e.g. correct/wrong feedback):
```js
MiniverseAudio.playSfx('correct');
MiniverseAudio.playBgm('lobby');   // looping, autoplay-safe
MiniverseAudio.toggleMute();        // persists to localStorage site-wide
```

> Note: every page nested under `games/<name>/` needs
> `MiniverseAudio.init({ basePath: '../../' })` so paths resolve back to
> the project root — see `games/direction-dash/index.html` or `game.html`
> for a working example.

Browsers block audio-with-sound from autoplaying until the user has
interacted with the page once — `playBgm()` already retries automatically
on the next click/keypress/tap, so the lobby music just starts a beat late
on a cold load rather than failing silently.
